              ...
              {invites?.map((invite) => {
                return (
                  <TableRow key={invite.id}>
                    ...
                    <TableCell className="py-2.5 font-medium">{invite.role}</TableCell>
                    <TableCell className="py-2.5">
                      <div className="flex justify-end">{permissions?.can('delete', 'Invite') && <RevokeInviteButton inviteId={invite.id} />}</div>
                    </TableCell>
                  </TableRow>
                )
              })}
