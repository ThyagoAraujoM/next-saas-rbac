...
import { getPendingInvites } from '@/src/http/get-pending-invites'
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-80 space-y-2">
        ...
        {data?.invites.length == 0 && <p className="text-muted-foreground text-sm">No invites found.</p>}
          ...
          return (
            <div key={invite.id} className="space-y-2">
              <p className="text-muted-foreground text-sm leading-relaxed">
                ...
                <span className="text-foreground font-medium">{invite.organization.name}</span> <span>{dayjs(invite.createdAt).fromNow()}</span>
              </p>
